...
void ParkingSystem::exportToHTML()
{
    ...
    file << "body { background: #f0f2f5; color: #2d3436; font-family: 'Nunito', sans-serif; text-align: center; margin: 0; padding: 20px; overflow-x: hidden; }";
    
    // Header: Modern, Clean, Dark Blue-Grey
    file << "h1 { color: #2d3436; font-weight: 800; font-size: 2.5em; margin-bottom: 10px; letter-spacing: -1px; }";
    file << "h1 span { color: #0984e3; }"; // Accent Color

    // Dashboard Container
    file << ".dashboard { display: flex; flex-wrap: wrap; justify-content: center; gap: 40px; margin-top: 30px; }";

    // Control Panel Card
    file << ".control-panel { background: #ffffff; border-radius: 20px; padding: 30px; min-width: 320px; max-width: 380px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); height: fit-content; text-align: left; transition: transform 0.3s ease; position: relative; z-index: 100; }";
    ...
    file << "button:active { transform: translateY(0); }";
    ...
    file << ".btn-diag { background: transparent; border: 2px dashed #b2bec3; color: #b2bec3; font-size: 0.8em; margin-top: 20px; box-shadow: none; }";
    file << ".btn-diag:hover { border-color: #0984e3; color: #0984e3; background: transparent; transform: none; }";
    ...
    // Free Slot: Subtle dotted border, minimal
    ...
    // Occ Slot: Solid color, friendly roundness
    file << ".occ { background: #fab1a0; color: #d63031; border: 2px solid #ff7675; animation: bounceIn 0.4s; }";
    file << "@keyframes bounceIn { 0% { transform: scale(0.3); opacity: 0; } 50% { transform: scale(1.05); } 70% { transform: scale(0.9); } 100% { transform: scale(1); opacity: 1; } }";
    ...
    file << "    if(!v) { alert('Whoops! Enter Vehicle ID please.'); return; }"; // Friendly Alert
